...
![Рабочая панель тренажёра](docs/screenshots/dashboard.png)

Скриншот локальной production-сборки в гостевом режиме: интерфейс и основной функционал доступны без регистрации.
...
GitHub Actions запускается на каждый push и pull request в `main`: устанавливает зависимости через `npm ci`, выполняет автоматические тесты (`npm test`) и проверяет production-сборку (`npm run build`). Сейчас в проекте 5 автоматических тестов для memory quiz. Статус виден в badge в начале README.
